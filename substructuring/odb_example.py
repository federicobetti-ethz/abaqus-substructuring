...
section = odb.HomogeneousSolidSection(
    name=section_name, material=material_name, thickness=2.0
)
...
element_set = instance.ElementSetFromElementLabels(
    name="AllElements-1", elementLabels=[elementData[0] for elementData in elementData]
)
instance.assignSection(region=element_set, section=section)
instance.NodeSetFromNodeLabels(
    name="AllNodes-1", nodeLabels=[nodeData[0] for nodeData in nodeData]
)

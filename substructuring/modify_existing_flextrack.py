...
def BASELINE_MAP_COUPLINGS(i):
    ...
    return {
        range_to_consider[0]: ["Coupling 1-1-{}-4".format(i), "Coupling 1-2-1-{}-1".format(i)],
        range_to_consider[1]: ["Coupling 1-1-{}-1".format(i), "Coupling 1-1-{}-2".format(i)],
...
def add_tying_surfaces_for_replication(src_model, part_name, num_copies):
    ...
    if num_copies > 1:
        for bc_name, bc_obj in src_model.boundaryConditions.items():
            ...
            region_elements_labels = [elem.label for node in region_set.nodes for elem in node.getElements()]
            region_elements_labels = list(dict.fromkeys(region_elements_labels))  # remove duplicates
            region_elements = base_instance.elements.sequenceFromLabels(region_elements_labels)
            region_elements_mask = region_elements.getMask()
            params = {"name": "{}".format(bc_name)}
            if str(bc_name)[-1] in ["1", "2"]:
                params["face1Elements"] = base_instance.elements.getSequenceFromMask(mask=region_elements_mask)
            elif str(bc_name)[-1] in ["3", "4"]:
                params["face2Elements"] = base_instance.elements.getSequenceFromMask(mask=region_elements_mask)
...
def define_model_materials(new_model):
    ...
    new_model.materials["bbr1_ENG-Stainless Steel (ferritic)"].Density(table=((7800.0,),))
    new_model.materials["bbr1_ENG-Stainless Steel (ferritic)"].Conductivity(table=((18.0,),))
    new_model.materials["bbr1_ENG-Stainless Steel (ferritic)"].Elastic(table=((2e11, 0.28),))
    new_model.materials["bbr1_ENG-Stainless Steel (ferritic)"].Expansion(table=((1.1e-5,),))

    new_model.materials['bbr1_ENG-Stainless Steel (ferritic)'].Plastic(hardening=JOHNSON_COOK,
...
def copy_sections_from_original_model(src_model, new_model, num_copies, part_name):
    ...
    for section_name, section_obj in src_model.sections.items():
        if section_name.startswith("Sec"):
            new_model.HomogeneousSolidSection(name=section_name, material=section_obj.material, thickness=section_obj.thickness)
            ...
            new_model.ConnectorSection(name=section_name, 
...
def copy_sets_and_internal_sets_from_original_model(src_asm, new_asm, num_copies, part_name):
    ...
    for set_name, aset in src_asm.sets.items():
        ...
        for i in range(num_copies):
            inst_name = "{}-{}".format(part_name, i+1)
            if set_mask:
                new_asm.Set(name="{}_{}".format(inst_name, set_name), nodes=new_asm.instances[inst_name].nodes.getSequenceFromMask(mask=set_mask))
    
    # all the NODE internal sets to the new model
    for internal_set_name, internal_aset in src_asm.allInternalSets.items():
        for i in range(num_copies):
            inst_name = "{}-{}".format(part_name, i+1)
            if internal_set_name.startswith("RAI") or internal_set_name.startswith("SL_") or internal_set_name.startswith("_M"):
                node_set_mask = internal_aset.nodes.getMask()
                new_asm.Set(name="{}_{}".format(inst_name, internal_set_name), nodes=new_asm.instances[inst_name].nodes.getSequenceFromMask(mask=node_set_mask), internal=ON)
    ...
    for internal_set_name, internal_aset in src_asm.allInternalSets.items():
        ...
        for i in range(num_copies):
            ...
            if internal_set_name.startswith("_Picked") and element_set_mask:
                new_asm.Set(name="{}_{}".format(inst_name, internal_set_name), elements=new_asm.instances[inst_name].elements.getSequenceFromMask(mask=element_set_mask), internal=ON)

def copy_surfaces_from_original_model(src_asm, new_asm, src_model, num_copies, part_name):
    ...
    for surf_name, asurf in src_asm.surfaces.items():
        ...
        if surf_name.startswith("TIE"):  # tie couplings between rail sections
            for i in range(num_copies):
                inst_name = "{}-{}".format(part_name, i+1)
                elements_to_tag = new_asm.instances[inst_name].elements.getSequenceFromMask(mask=face_elements.getMask())
                if str(asurf.sides[0]) == "FACE1":
                    new_asm.Surface(name="{}_{}".format(inst_name, surf_name), face1Elements=elements_to_tag)
                elif str(asurf.sides[0]) == "FACE2":
                    new_asm.Surface(name="{}_{}".format(inst_name, surf_name), face2Elements=elements_to_tag)
                elif str(asurf.sides[0]) == "FACE4":
                    new_asm.Surface(name="{}_{}".format(inst_name, surf_name), face4Elements=elements_to_tag)
        
        elif surf_name.startswith("BAL"):  # ballast connections
            str_asurf_sides = [str(side) for side in asurf.sides]
            face4_tags = [index for index, element in enumerate(str_asurf_sides) if element == "FACE4"]
            face5_tags = [index for index, element in enumerate(str_asurf_sides) if element == "FACE5"]
            face4_mask = face_elements[face4_tags[0] : face5_tags[-1] + 1].getMask()
            face5_mask = face_elements[face5_tags[0] : face5_tags[-1] + 1].getMask()
            for i in range(num_copies):
                inst_name = "{}-{}".format(part_name, i+1)
                elements_to_tag_face4 = new_asm.instances[inst_name].elements.getSequenceFromMask(mask=face4_mask)
                elements_to_tag_face5 = new_asm.instances[inst_name].elements.getSequenceFromMask(mask=face5_mask)
                new_asm.Surface(
            ...
            for i in range(num_copies):
                ...
                face_mask = src_model.rootAssembly.surfaces[surf_name].elements.getMask()
                if str(surf_name)[-1] in ["1", "2"]:
                    base_params["face1Elements"] = inst_instance.elements.getSequenceFromMask(mask=face_mask)
                elif str(surf_name)[-1] in ["3", "4"]:
                    base_params["face2Elements"] = inst_instance.elements.getSequenceFromMask(mask=face_mask)
...
def define_and_remove_redundant_bcs(src_model, new_model, num_copies, part_name):
    ...
    for bc_name, bc_obj in src_model.boundaryConditions.items():
        ...
        for i in range(num_copies):
            ...
            region = rts.Region(nodes=inst_instance.nodes.getSequenceFromMask(mask=bc_nodes_mask))
...
def copy_constraints_from_original_model(src_model, new_model, num_copies, part_name):
    ...
    for constr_name, constr_obj in src_model.constraints.items():
        if constr_name.startswith("Cou"):
            control_point_mask = src_model.rootAssembly.sets[constr_obj.controlPoint[0]].nodes.getMask()
            surface_string = constr_obj.surface[0]
            for i in range(num_copies):
                inst_name = "{}-{}".format(part_name, i+1)
                control_point_region = rts.Region(nodes=new_model.rootAssembly.instances[inst_name].nodes.getSequenceFromMask(mask=control_point_mask))
                ...
                try:
                    base_coupling_params["surface"] = new_model.rootAssembly.surfaces["{}_{}".format(inst_name, surface_string)]
                    new_model.Coupling(**base_coupling_params)
                except:
                    base_coupling_params["surface"] = new_model.rootAssembly.sets["{}_{}".format(inst_name, surface_string)]
                    new_model.Coupling(**base_coupling_params)                    
        elif constr_name.startswith("TIE"):
            for i in range(num_copies):
                inst_name = "{}-{}".format(part_name, i+1)
                new_model.Tie(name="{}_{}".format(inst_name, constr_name),
                    main=new_model.rootAssembly.surfaces["{}_{}".format(inst_name, constr_obj.main[0])],
                    secondary=new_model.rootAssembly.surfaces["{}_{}".format(inst_name, constr_obj.secondary[0])],
...
def copy_connector_assignments_from_original_model(src_model, new_model, num_copies, part_name):
    ...
    for i in range(num_copies):
        ...
        for j, _ in enumerate(src_model.rootAssembly.sectionAssignments):
            ...
            constraint_names = tuple(["{}_{}".format(inst_name, constraints_to_consider[k]) for k in range(len(constraints_to_consider))])
            
            nodes = []
            for constraint_name in constraint_names:
                control_point_nodes = new_model.rootAssembly.allInternalSets[new_model.constraints[constraint_name].controlPoint[0]].nodes
                try:
                    nodes.append(instance_nodes[control_point_nodes[0].label-1])
                except:
                    closest_node = instance_nodes.getClosest(coordinates=control_point_nodes[0].coordinates, numToFind=2, searchTolerance=0.01)
                    nodes.append(closest_node[0] if j % 2 != 0 else closest_node[1])
            nodes = [nodes[0], None] if len(nodes) == 1 else nodes
            new_model.rootAssembly.WirePolyLine(points=((nodes[0], nodes[1]),), mergeType=IMPRINT, meshable=False)

            new_model.rootAssembly.DatumCsysByThreePoints(
                name="{}_{}".format(inst_name, src_model.rootAssembly.features[src_model.rootAssembly.features.keys()[j+1]].name),
            ...
            section_assignment_number = (edge_feature_num - 1) % num_original_model_wires

            section_assignment_name = src_model.rootAssembly.sectionAssignments[section_assignment_number].sectionName
            
            new_model.rootAssembly.SectionAssignment(
                region=region,
                sectionName=section_assignment_name
            )

            new_model.rootAssembly.ConnectorOrientation(
                region=new_model.rootAssembly.allInternalSets[new_model.rootAssembly.allInternalSets.keys()[-1]],
                axis1=AXIS_1,
                axis2=AXIS_1,
                angle1=0.0,
                angle2=0.0,
...
def create_tying_constraints_between_instances(new_model, num_copies, part_name):
    ...
    for i in range(num_copies - 1):
        ...
        new_model.Tie(name="{}-TIE-{}-1".format(main_inst_name, secondary_inst_name),
            main=new_model.rootAssembly.surfaces["{}_Disp-BC-4".format(main_inst_name)],
            secondary=new_model.rootAssembly.surfaces["{}_Disp-BC-1".format(secondary_inst_name)],
            constraintEnforcement=SURFACE_TO_SURFACE,
            tieRotations=ON,
            positionToleranceMethod=COMPUTED,
            thickness=ON,
        ...
        new_model.Tie(name="{}-TIE-{}-2".format(main_inst_name, secondary_inst_name),
            main=new_model.rootAssembly.surfaces["{}_Disp-BC-3".format(main_inst_name)],
            secondary=new_model.rootAssembly.surfaces["{}_Disp-BC-2".format(secondary_inst_name)],
            constraintEnforcement=SURFACE_TO_SURFACE,
            tieRotations=ON,
            positionToleranceMethod=COMPUTED,
            thickness=ON,
...
def define_frequency_extraction_job(new_model_name, num_eigenmodes=50, solve=True):
    ...
    mdb.jobs['FrequencyExtraction'].setValues(multiprocessingMode=MPI, numCpus=4, numDomains=4, numThreadsPerMpiProcess=1)
...
def copy_part_with_assembly_sets_surfaces(
    ...
    copy_sets_and_internal_sets_from_original_model(src_asm, new_asm, num_copies, part_name)
    copy_surfaces_from_original_model(src_asm, new_asm, src_model, num_copies, part_name)
    ...
    copy_connector_assignments_from_original_model(src_model, new_model, num_copies, part_name)

...
for i in range(num_substructures):
    ...
    for node in instance.nodes:
        if (
            node.coordinates[0] > i * L / num_substructures
            and node.coordinates[0] < (i + 1) * L / num_substructures
        ):
            internal_nodes.append(node.label)
        elif np.isclose(node.coordinates[0], i * L / num_substructures) or np.isclose(
            node.coordinates[0], (i + 1) * L / num_substructures
        ):
...
for i in range(num_substructures):
    ...
    internal_dofs = internal_node_labels[:, np.newaxis] * n_dofs_per_node + np.arange(
        n_dofs_per_node
    )
    surface_dofs = surface_node_labels[:, np.newaxis] * n_dofs_per_node + np.arange(
        n_dofs_per_node
    )
